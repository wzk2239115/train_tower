...
from tower.train.vram_tune import apply_h800_vram_tune, peak_vram_score, _get_anchor
...
def test_max_uw_clamped_to_stable_envelope(monkeypatch):
    ...
    assert peak_vram_score(cfg, anchor) > 1.0

    tuned = apply_h800_vram_tune(cfg)
    assert peak_vram_score(tuned, anchor) <= 1.0 + 1e-9
    assert tuned.per_device_train_batch_size == 8
    assert tuned.max_pixels == 4_194_304
    assert tuned.gradient_accumulation_steps == 3  # target 160/8 = 20 per GPU
