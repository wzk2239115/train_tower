...
# Optional local tokenizer dir (override: export LOCAL_TOKENIZER=/path/to/tokenizer)
LOCAL_TOKENIZER="${LOCAL_TOKENIZER:-}"
if [[ -n "${LOCAL_TOKENIZER}" && -f "${LOCAL_TOKENIZER}/vocab.json" ]]; then
  echo "Copying tokenizer from ${LOCAL_TOKENIZER} ..."
  for f in tokenizer.json tokenizer_config.json vocab.json merges.txt special_tokens_map.json added_tokens.json chat_template.jinja; do
    [[ -f "${LOCAL_TOKENIZER}/${f}" ]] && cp "${LOCAL_TOKENIZER}/${f}" "${OUT}/"
  done
else
